...
export default function TransactionList({ transactions, onEdit, onDeleted, limit }: Props) {
  ...
  return (
    <div className="space-y-2">
      {items.map((t) => (
        <div key={t.id} className="flex items-center justify-between bg-white rounded-lg border border-gray-100 px-4 py-3 hover:shadow-sm transition-shadow">
          <div className="flex items-center gap-3">
            <div className="w-2 h-10 rounded-full" style={{ backgroundColor: t.category_color }} />
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            {onEdit && <button onClick={() => onEdit(t)} className="text-xs text-gray-400 hover:text-indigo-600 px-1">編集</button>}
            {onDeleted && <button onClick={() => handleDelete(t.id)} className="text-xs text-gray-400 hover:text-red-500 px-1">削除</button>}
          </div>
        </div>
      ))}
    </div>
